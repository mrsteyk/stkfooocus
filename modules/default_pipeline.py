...
def refresh_refiner_model(name):
    ...
    if name == "None":
        ...
        core.comfy.model_management.soft_empty_cache()
        return
    ...
    xl_refiner = core.load_model_refiner(filename)
...
def refresh_base_with_loras(base, loras):
    global xl_base_patched, xl_base_patched_hash
    if xl_base_patched_hash == str(base) + str(loras):
        return
    ...
    # Clear old model from memory before (re)loading a new one
    xl_base_patched = None
    core.comfy.model_management.soft_empty_cache()

    filename = os.path.join(modules.path.modelfile_path, base)
    ...
    print(f"Base model loaded: {base}")
    ...
    xl_base_patched_hash = str(base) + str(loras)
...
refresh_base_with_loras(
    modules.path.default_base_model_name,
    [
        (modules.path.default_lora_name, 0.5),
        ("None", 0.5),
        ("None", 0.5),
        ("None", 0.5),
        ("None", 0.5),
