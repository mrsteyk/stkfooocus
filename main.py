...
from modules.sdxl_styles import style_keys, aspect_ratios  # noqa
...
def generate_clicked(*args):
    ...
    while not finished:
        ...
        if len(worker.outputs) > 0:
            ...
            if flag == "results":
                yield gr.update(interactive=True), gr.update(visible=False), gr.update(
                    visible=False
...
if __name__ == "__main__":
    ...
    # Don't use xformers (useless in this context?)
    sys.argv += ["--use-pytorch-cross-attention"]

    shared.gradio_root = gr.Blocks(title="Fooocus STK", css=modules.html.css).queue()
    with shared.gradio_root:
        with gr.Row():
            ...
            with gr.Column(scale=0.5, visible=False) as right_col:
                with gr.Tab(label="Setting"):
                    performance_selction = gr.Radio(
                        label="Performance", choices=["Speed", "Quality"], value="Speed"
                    )
                    aspect_ratios_selction = gr.Radio(
                        label="Aspect Ratios (width × height)",
                        choices=list(aspect_ratios.keys()),
                        value="1152×896",
                    )
                    image_number = gr.Slider(
                        label="Image Number", minimum=1, maximum=32, step=1, value=2
                    )
                        ...
                        for i in range(5):
                            with gr.Row():
                                lora_model = gr.Dropdown(
                                    ...
                                    value=modules.path.default_lora_name
                                    if i == 0
                                    else "None",
                                )
            ...
            run_button.click(
                fn=generate_clicked,
                inputs=ctrls,
                outputs=[run_button, progress_html, progress_window, gallery],
            )

    shared.gradio_root.launch(
        inbrowser=True, server_name=args.listen, server_port=args.port, share=args.share
    )
